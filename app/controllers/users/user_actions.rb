module Users
  module UserActions
    ...
    def followers
      @users = @user.follow_by_users.normal.order("actions.id asc").paginate(page: params[:page], per_page: 60)
      fresh_when([@users])
    end

    def following
      @users = @user.follow_users.normal.order("actions.id asc").paginate(page: params[:page], per_page: 60)
